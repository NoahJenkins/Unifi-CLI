...
func newDoctorCmd(info buildinfo.Info) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return doctorError(rt, apperr.WithHint(
			...
			if err != nil {
				return doctorError(rt, apperr.WithCause(
					apperr.WithHint(
			...
			if info.Version == "" || info.Commit == "" {
				return doctorError(rt, apperr.New(apperr.Internal, "version metadata is incomplete"))
			}
			result.Ready = true
			return emittedExit(rt.Emit("doctor", "check", result, nil, nil))
		},
	}
}
...
func doctorError(rt *Runtime, err error) error {
	return emittedExit(rt.Emit("doctor", "check", nil, nil, err))
}
